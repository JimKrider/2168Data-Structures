    ...
    public static void main(String[] args)
     {
        try
        {
 	   String s; int i, j, n, numfiles, c, first, last;
 	   numfiles=args.length; first=(int) 'a'; last=(int) 'z';
 	   ...
 	   for (j=0; j<numfiles-1; j=j+2)
 		 ...
 		 String lower=s.toLowerCase(); n=lower.length();
 		 for (i=0; (i<n-1)&&yes; i++) 
                 {
                    c=(int) lower.charAt(i); 
                    if ((first>c)||(c>last)) yes=false;
                 }
                 c=(int) lower.charAt(n-1); char e=s.charAt(n-1);
 	         ...
 	         if (yes) 
 		 {
             ...
             AllWords.insert(s); allstop=System.currentTimeMillis();
 		     allelapsed=allelapsed+allstop-allstart;
 		 }
   	      }
 	      System.out.print(elapsed/1000.00); System.out.println(" - "+args[j]);
 	   ...
 	   System.out.print(allelapsed); System.out.println(
              " - "+args[args.length-1]);
    	}
